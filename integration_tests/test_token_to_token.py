...
class TokenToTokenTest(TestCase):
    ...
    def test_tt_uninitialized(self):
        with self.assertRaises(MichelsonRuntimeError):
            res = self.dex.invest(pair_id=0, token_a_in=10_000, token_b_in=10_000, shares=10_000).interpret(amount=1)
    ...
    def test_tt_fee_is_distributed_evenly(self):
        ...
        # divest bob's shares
        res = chain.execute(self.dex.divest(pair_id=0, min_token_a_out=1, min_token_b_out=1, shares=100_000), sender=bob)
    ...
    def test_tt_multiple_singular_invests(self):
        ...
        res = chain.execute(self.dex.divest(0, 1, 1, 3))
    ...
    def test_tt_multiple_small_invests(self):
        ...
        for ratio in ratios:
            ...
            for i in range(3):
                ...
                res = chain.execute(self.dex.invest(pair_id=0, token_a_in=100, token_b_in=token_b_amount, shares=shares))

            all_shares = res.storage["storage"]["ledger"][(me,0)]["balance"]

            res = chain.execute(self.dex.divest(pair_id=0, min_token_a_out=1, min_token_b_out=1, shares=all_shares))
    ...
    def test_tt_divest_big_a_small_b(self):
        ...
        with self.assertRaises(MichelsonRuntimeError):
            res = chain.execute(self.dex.invest(pair_id=0, token_a_in=2_000_000 - 1, token_b_in=1, shares=1))
    ...
    def test_tt_divest_small_a_big_b(self):
        ...
        with self.assertRaises(MichelsonRuntimeError):
            res = chain.execute(self.dex.invest(pair_id=0, token_a_in=1, token_b_in=2_000_000 - 1, shares=1))
    ...
    def test_tt_reinitialize(self):
        ...
        res = chain.execute(self.dex.divest(0, 1, 1, 10))

        # following fails since pair is considered uninitialized
        with self.assertRaises(MichelsonRuntimeError):
            res = chain.execute(self.dex.invest(0, 10, 10, 1))
    ...
    def test_tt_divest_smallest(self):
        ...
        res = chain.execute(self.dex.divest(0, 1, 1, 2))
        transfers = parse_token_transfers(res)
        ...
        res = altchain.execute(self.dex.divest(0, 1, 1, 3), sender=alice)
        transfers = parse_token_transfers(res)
    ...
    def test_tt_invest_min_b_shares(self):
        ...
        res = chain.execute(self.dex.invest(pair_id=0, token_a_in=100_000, token_b_in=100, shares=100))
        with self.assertRaises(MichelsonRuntimeError):
            res = chain.execute(self.dex.invest(pair_id=0, token_a_in=100_000, token_b_in=100, shares=101))
    ...
    def test_tt_invest_smallest(self):
        ...
        with self.assertRaises(MichelsonRuntimeError):
           res = chain.execute(self.dex.invest(pair_id=0, token_a_in=2, token_b_in=3, shares=3))
