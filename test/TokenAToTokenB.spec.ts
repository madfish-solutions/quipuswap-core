...
contract("TokenAToTokenB()", function () {
  ...
  function tokenAToTokenBSuccessCase(
    ...
    it(decription, async function () {
      ...
      const aliceInitTezLedger = await context.tokens[0].storage.ledger[
        aliceAddress
      ];
      const aliceInitTezBalance = aliceInitTezLedger
        ? aliceInitTezLedger.balance
        : new BigNumber(0);
      const aliceInitTokenLedger = await context.tokens[1].storage.ledger[
        aliceAddress
      ];
      const aliceInitTokenBalance = aliceInitTokenLedger
        ? aliceInitTokenLedger.balance
        : new BigNumber(0);
      const bobInitTokenLedger = await context.tokens[1].storage.ledger[
        bobAddress
      ];
      const bobInitTokenBalance = bobInitTokenLedger
        ? bobInitTokenLedger.balance
        : new BigNumber(0);
      const prevPairTokenBalance = await context.tokens[1].storage.ledger[
        pairAddress
      ].balance;
      const prevPairTezBalance = await context.tokens[0].storage.ledger[
        pairAddress
      ].balance;
      ...
      const aliceFinalTezLedger = await context.tokens[0].storage.ledger[
        aliceAddress
      ];
      const aliceFinalTezBalance = aliceFinalTezLedger
        ? aliceFinalTezLedger.balance
        : new BigNumber(0);
      const aliceFinalTokenLedger = await context.tokens[1].storage.ledger[
        aliceAddress
      ];
      const aliceFinalTokenBalance = aliceFinalTokenLedger
        ? aliceFinalTokenLedger.balance
        : new BigNumber(0);
      const bobFinalTokenLedger = await context.tokens[1].storage.ledger[
        bobAddress
      ];
      const bobFinalTokenBalance = bobFinalTokenLedger
        ? bobFinalTokenLedger.balance
        : new BigNumber(0);
      const pairTokenBalance = await context.tokens[1].storage.ledger[
        pairAddress
      ].balance;
      const pairTezBalance = await context.tokens[0].storage.ledger[pairAddress]
        .balance;
      strictEqual(
      ...
      strictEqual(
        ...
        prevStorage.pairs[0].token_b_pool.toNumber() -
          tokenBAmount -
          tokensLeftover
      );
      strictEqual(
        context.dex.storage.pairs[0].token_a_pool.toNumber(),
        prevStorage.pairs[0].token_a_pool.toNumber() + tokenAAmount
      );
    });
  }
