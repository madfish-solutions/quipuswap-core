...
describe("TokenToTezPayment()", function () {
  it.only("should exchnge token to tez and update dex state", async function () {
    ...
    let carolInitTezBalance = await context.tezos.tz.getBalance(carolAddress);
    ...
    let bobFinalTokenBalance = bobFinalTokenLedger
      ...
      : new BigNumber(0);
    let carolFinalTezBalance = await context.tezos.tz.getBalance(carolAddress);
