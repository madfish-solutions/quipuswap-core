...
function tokenToTokenOut (const gs : gateway_storage; var s: dex_storage) :  list(operation) is
 block {
   ...
   transaction(Lookup(gs.tmp.2, gs.sender, gs.tmp.1, tezOut), 
   0tz,
   case (Tezos.get_entrypoint_opt("%lookup", gs.main) : option(contract(w))) of Some(contr) -> contr
...
function main (const p : gatewayAction ; const s : gateway_storage) :
  ...
  | ReceiveDexStorage(n) -> (tokenToTokenOut(s, n), s) 
