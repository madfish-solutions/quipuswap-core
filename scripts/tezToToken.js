...
exec(
  `ligo compile-parameter ./contracts/Dex.ligo main -s pascaligo "TezToTokenSwap(${minTokens}n)"`,
  async (error, stdout, stderr) => {
    ...
    exec(
      `./babylonnet.sh client -A rpcalpha.tzbeta.net -P 443 -S  transfer ${tezAmount} from alice to "${dexAddress}" --arg "${stdout.trim()}" --burn-cap 0.5`,
      async (error, stdout, stderr) => {
        console.log(stdout);
        sleep(50000).then(() => {
          exec(
