...
exec(`ligo compile-parameter ./contracts/Token.ligo main -s pascaligo "Approve((\\\"${dexAddress}\\\" : address), ${allowedAmount}n)"`, async (error, stdout, stderr) => {
  if (error) {
  ...
  exec(`./babylonnet.sh client -A rpcalpha.tzbeta.net -P 443 -S  transfer 0 from alice to "${tokenAddress}" --arg "${stdout.trim().replace(/\"/g, "\\\"")}" --burn-cap 0.5`, async (error, stdout, stderr) => {
    console.log(stdout);
    sleep(50000).then( () => {
      exec(`ligo compile-parameter ./contracts/Dex.ligo main -s pascaligo "InvestLiquidity(${shares}n)"`, (error, stdout, stderr) => {
        if (error) {
            console.error(`exec error: ${error}`);
            return;
        }
