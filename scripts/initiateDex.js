...
exec(`ligo compile-parameter ./contracts/Token.ligo main -s pascaligo "Approve((\\\"${dexAddress}\\\" : address), ${amount}n)"`, async (error, stdout, stderr) => {
  if (error) {
  ...
  exec(`./babylonnet.sh client -A rpcalpha.tzbeta.net -P 443 -S  transfer 0 from alice to "${tokenAddress}" --arg "${stdout.trim().replace(/\"/g, "\\\"")}" --burn-cap 0.5`, async (error, stdout, stderr) => {
    ...
    sleep(100000).then( () => {
      exec(`ligo compile-parameter ./contracts/Dex.ligo main -s pascaligo "InitializeExchange(${amount}n)"`, (error, stdout, stderr) => {
        ...
        exec(`./babylonnet.sh client -A rpcalpha.tzbeta.net -P 443 -S  transfer 1 from alice to "${dexAddress}" --arg "${stdout.trim()}" --burn-cap 0.5`, (error, stdout, stderr) => {
          console.log(stdout);
