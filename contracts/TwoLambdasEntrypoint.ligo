...
#include "ITwoLambdasEntrypoint.ligo"
...
function setFirstFunctionsSet (const initializeInvestF : big_map(nat,(address * nat * dex_storage) -> (list(operation) * dex_storage));
   const tezToTokenF : big_map(nat, (address * address * nat * nat * dex_storage) -> (list(operation) * dex_storage));
 var s: full_dex_storage ) :  (full_dex_storage) is
 block {
    s.initializeInvest := initializeInvestF;
    s.tezToToken := tezToTokenF;
 } with s
...
//   | TokenToTokenSwap(n) -> tokenToTokenOutMiddle(Tezos.sender, Tezos.sender, this, n.0, n.1, n.2, s)
  ...
  | SetFirstFunctionsSet(n) -> ((nil: list(operation)), setFirstFunctionsSet(n.0, n.1, s))
 end
