...
function setFirstFunctionsSet (const initializeInvestF : (address * nat * dex_storage) -> (list(operation) * dex_storage);
 var s: full_dex_storage ) :  (full_dex_storage) is
 block {
   // Two ways to fix issue found& 
   // Option 1: Replace block with skip.
   // Option 2: Merge initializeExchange and investLiquidity into one big_map(WTF???) 
    s.initializeExchange[0n] := initializeInvestF;
 } with s
...
//   | TokenToTokenSwap(n) -> tokenToTokenOutMiddle(Tezos.sender, Tezos.sender, this, n.0, n.1, n.2, s)
  ...
  | SetFirstFunctionsSet(n) -> ((nil: list(operation)), setFirstFunctionsSet(n, s))
 end
